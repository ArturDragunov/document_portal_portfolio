...
FAISS_INDEX_PATH = Path("faiss_index") # inside this folder, Faiss index will be available

def test_conversational_rag_on_pdf(pdf_path:str, question:str):
    try:
        model_loader = ModelLoader()
        # Check for actual FAISS files, not just the folder
        faiss_files_exist = (FAISS_INDEX_PATH / "index.faiss").exists() and (FAISS_INDEX_PATH / "index.pkl").exists()
        if faiss_files_exist:
            print("Loading existing FAISS index...")
            embeddings = model_loader.load_embeddings()
            # allow_dangerous_deserialization=True when loading a pickle file -> as it can be modified by other users
            # our faiss index will be saved in a pickle format on our disk
        ...
        else: # if FAISS is not available, then do embedding and indexing yourself
...
if __name__ == "__main__":
    ...
    question = "What is attention about?"
